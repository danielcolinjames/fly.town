...
const { disconnect, insertData, database } = require('../../packages/db/connect')
  ...
  try {
    ...
    if (response.status === 200 && response.data.records) {
      ...
      return response.data.records.map(record => ({
        ...record,
        restaurant_id: generateRestaurantId(record.restaurant_name),
...
async function fetchAndInsertIPFSData(newEvents) {
  ...
  if (allIPFSData.length > 0) {
    ...
    await insertData(DB_NAME, 'checkins', allIPFSData)
