...
async function findHighestTokenID() {
  ...
  const batchSignature = ethers.utils.id(
  ...
  for (
    ...
    console.log(
      `Fetching logs from block ${startBlock} to ${toBlock} for address ${nftCa}`,
    );
    ...
    try {
      ...
      for (const event of batches) {
        ...
        const currentHighestId = ids.reduce(
          (max, id) =>
            ethers.BigNumber.from(id).gt(max) ? ethers.BigNumber.from(id) : max,
      ...
      console.log(
        `Highest ID found in the last ${toBlock - fromBlock} blocks:`,
        highestId.toNumber(),
      );
      return highestId;
    } catch (error) {
...
async function fetchNFTMetadataInBatches(
  fromTokenId,
  toTokenId,
  batchSize = 100,
) {
  ...
  const highestMembershipId = highestMembershipDoc
    ? parseInt(highestMembershipDoc.membershipId, 10)
    : 0;
  ...
  for (let i = fromTokenId; i <= toTokenId; i += batchSize) {
    const batchIds = Array.from(
      { length: Math.min(batchSize, toTokenId - i + 1) },
      (_, index) => i + index,
    );
    console.log(
      "Beginning fetching metadata for NFT ID range:",
      batchIds[0],
      "to",
  ...
  console.log(
    `Fetched metadata for ${effectiveRange} IDs, NFTs, skipping ${skippedIds} existing IDs`,
  );
...
function reformatNFTMetadata(nftMetadata) {
  const restaurantName = nftMetadata.attributes.find(
    (attr) => attr.trait_type === "restaurantName",
  ).value;
  const reformattedMetadata = {
    restaurantName: restaurantName,
    restaurantId: generateRestaurantId(restaurantName),
    location: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "restaurantLocations",
    ).value,
    imageArtist: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "artist",
    ).value,
    generation: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "generation",
    ).value,
    expirationDate: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "expirationDate",
    ).value,
    mintDate: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "mintDate",
    ).value,
    memberStatus: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "memberStatus",
    ).value,
    accessLevel: nftMetadata.attributes.find(
      (attr) => attr.trait_type === "accessLevel",
    ).value,
...
async function writeMembershipsToDatabase(nftMetadata) {
  ...
  await db
    ...
    .createIndex({ membershipId: 1 }, { unique: true });
  ...
  for (const item of nftMetadata) {
    ...
    if (!item.restaurantName || !item.restaurantId) {
      console.log(
        `Skipping item with blank restaurantName or restaurantId: ${JSON.stringify(item)}`,
      );
...
async function findUniqueRestaurants(nftMetadata) {
  ...
  for (const nftData of nftMetadata) {
    if (!nftData || !nftData.restaurantName || !nftData.restaurantId) {
      errors.push({
        error: "Invalid NFT data or restaurantName/restaurantId missing",
        data: nftData,
      });
      continue;
    }
...
async function writeRestaurantsToDatabase(uniqueRestaurants) {
  ...
  await db
    ...
    .createIndex({ restaurantId: 1 }, { unique: true });
  ...
  for (const restaurant of uniqueRestaurants) {
    ...
    if (!restaurant.restaurantName || !restaurant.restaurantId) {
      console.log(
        `Skipping restaurant with blank name or restaurantId: ${JSON.stringify(restaurant)}`,
      );
      continue; // Skip this iteration if either field is blank
    }
    ...
    try {
      ...
      await insertData(DB_NAME, "restaurantErrors", {
        error,
        data: restaurant,
      });
    }
...
async function writeIPFSHashesToPendingHashesCollection() {
  ...
  const latestCheckIn = await db
    .collection("checkIns")
    .findOne({}, { sort: { "metadata.blockNumber": -1 } });
  if (
    latestCheckIn &&
    latestCheckIn.metadata &&
    latestCheckIn.metadata.blockNumber
  ) {
  ...
  console.log(
    `Beginning to check events from block ${startBlock} to block ${toBlock}...`,
  );
  console.log(
  ...
  for (
    ...
    try {
      ...
      const pendingHashes = chunkEvents.map((event) => {
        ...
        ipfsURI = ipfsURI.startsWith("ipfs://")
          ? ipfsURI.substring(7)
          : ipfsURI;
        return {
          ipfsHash: ipfsURI,
      ...
      const uniquePendingHashes = pendingHashes.filter(
        (pendingHash) =>
          !existingHashes.some(
            (existingHash) => existingHash.ipfsHash === pendingHash.ipfsHash,
          ),
      );
      ...
      if (uniquePendingHashes.length === 0) {
        ...
        console.log(
          `Inserted ${pendingHashes.length} records into pendingHashes collection`,
        );
      }
      ...
      console.error(
        `Error fetching logs for blocks ${startBlock} to ${toBlock}:`,
        error,
      );
    }
  }
...
async function fetchAndPublishIPFSDataInBatches(hashes, batchSize = 50) {
  ...
  await db
    .collection("checkIns")
    .createIndex({ checkInId: 1 }, { unique: true });
  ...
  for (let i = 0; i < hashes.length; i += batchSize) {
    ...
    console.log(
      `Processing batch ${Math.ceil(i / batchSize) + 1} of IPFS data...`,
    );

    for (const { ipfsHash, blockNumber, transactionHash } of batch) {
      const records = await fetchIPFSData(
        ipfsHash,
        blockNumber,
        transactionHash,
      );
      for (const record of records) {
        ...
        if (!record.checkInId || record.checkInId === "") {
          console.log(
            `Skipping record with blank checkInId: ${JSON.stringify(record)}`,
          );
          continue;
        }
  ...
  try {
    ...
    if (response.status === 200) {
      return response.data.records.map((record) =>
        reformatCheckInData(record, blockNumber, transactionHash, hash),
      );
    } else {
      console.error(
  ...
  for (const restaurantId of restaurantIds) {
    ...
    for (const level in restaurant.accessLevels) {
      ...
      if (latestMembership) {
        ...
        await db
          .collection("restaurants")
          .updateOne({ restaurantId }, { $set: updateFields });

        console.log(
          `Updated restaurant ${restaurantId} level ${level} with latest membership details.`,
        );
      }
    }
  }
...
async function enhanceRestaurants() {
  // await initializeTiers()
  const db = await database();
  const uniqueRestaurants = await db
    .collection("memberships")
    .distinct("restaurantId");
...
async function initializeTiers() {
  const db = await database();
  const uniqueRestaurantIds = await db
    .collection("memberships")
    .distinct("restaurantId");
  ...
  for (const restaurantId of uniqueRestaurantIds) {
    ...
    for (const membership of memberships) {
      const { accessLevel, image, imageArtist, memberStatus, location } =
        membership;
    ...
    const updateResult = await db
      ...
      .updateOne(
        { restaurantId },
        { $set: { accessLevels, location: restaurantLocation } },
        { upsert: true },
      );

    console.log(
      `Updated restaurantId ${restaurantId} with access levels:`,
      updateResult,
    );
  }
}
...
async function updateRestaurantTierDetails(restaurantId) {
  ...
  for (const level in restaurant.accessLevels) {
    ...
    try {
      ...
      console.log(
        `Updating restaurantId: ${restaurantId}, level: ${level} with:`,
        updateObject,
      );
      await db
        .collection("restaurants")
        .updateOne({ restaurantId }, { $set: updateObject });
    } catch (error) {
      console.error(
        `Error updating restaurantId: ${restaurantId}, level: ${level}`,
        error,
      );
    }
  }
...
async function updateTierLevelCounts(restaurantId) {
  ...
  for (const { _id, count } of countsPerAccessLevel) {
    console.log(
      `Updating restaurantId: ${restaurantId}, level: ${_id} with count: ${count}`,
    );
    await db
      .collection("restaurants")
      .updateOne(
...
async function calculateGlobalStats() {
  ...
  const checkInsToday = await db
    .collection("checkIns")
    .countDocuments({ created_at: { $gte: startOfTodayUTC } });
