...
export async function getData(restaurantId: string): Promise<{
  ...
  try {
    ...
    const firstCheckinDate =
      checkinCount > 0 ? checkins[checkinCount - 1].createdAtDate : null;
    const mostRecentCheckinDate =
      checkinCount > 0 ? checkins[0].createdAtDate : null;
    const checkinsLast24h = checkins.filter(
      (c) => c.createdAtDate >= oneDayAgo,
    ).length;
    const checkinsLastMonth = checkins.filter(
      (c) => c.createdAtDate >= oneMonthAgo,
    ).length;
    ...
    const numberOfMemberships =
      numberOfMembershipsResult.length > 0
        ? numberOfMembershipsResult[0].numberOfMemberships
        : 0;
    let averageCheckinsPerMembership =
      numberOfMemberships > 0 ? checkinCount / numberOfMemberships : 0;
