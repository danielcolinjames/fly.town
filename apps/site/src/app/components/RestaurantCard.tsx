...
export const RestaurantCard = ({ hideCrown, restaurant, index, subtitle }: { hideCrown?: boolean, restaurant: Restaurant; index: number, subtitle?: string }) => {
  ...
  return (
    <Link href={`/${restaurant.restaurantId}`}>
      {/* <motion.div */}
      <div
        style={index === 0 ? { boxShadow: `0 0 100px ${accentColor}12`, borderColor: `${accentColor}40` } : {}}
        className={classNames(
          'flex flex-row hover:bg-[#040404] duration-200 transition-all rounded-full w-full border',
          index === 0 ? 'bg-[#050502] hover:bg-black' : 'bg-[#0a0a0a] border-[#202020]',
        )}
      // style={{ borderColor: `${accentColor}24` }}
      // layoutId={`restaurant-card-${restaurant.restaurantId}`}
      >
        <div className='w-full flex flex-row items-center justify-start gap-3 sm:gap-4 px-2 sm:px-2 py-2 rounded-full' style={index === 0 ? { boxShadow: `0 0 15px ${accentColor}25` } : {}}>
          {/* <motion.img */}
          <Image
            src={imageUrl}
        ...
        /> */}
          <div className="flex flex-col items-start sm:items-start">
            <div className="flex items-center">
              ...
              {index === 0 && !hideCrown && <Crown className="-mt-10 sm:-mt-14 size-6 sm:size-8 rotate-[24deg] text-brandYellow" />}
            </div>
